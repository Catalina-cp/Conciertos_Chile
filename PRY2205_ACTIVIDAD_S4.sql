...
SELECT
 ...
 TO_CHAR(ROUND(t.SUELDO_BASE), '999G999G999') AS "Sueldo Base"
...
SELECT
 ...
 TO_CHAR(SUM(NVL(ct.VALOR_COMISION, 0)), '999G999G999') AS "Comisión Total",
  UPPER(tt.DESC_CATEGORIA) AS "Tipo Trabajador",
 UPPER(cc.NOMBRE_CIUDAD) AS "Ciudad Trabajador"
...
GROUP BY 
 ...
 tt.DESC_CATEGORIA, cc.NOMBRE_CIUDAD
...
SELECT
 ...
 EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM t.FECING) "Años Antigüedad",
 ...
 '$' || TO_CHAR(
   CASE
      ...
      t.FECING) <= 10 THEN t.SUELDO_BASE * 0.10
...
FROM TRABAJADOR t
    ...
    JOIN EST_CIVIL ev ON t.NUMRUT = ev.NUMRUT_T
    JOIN ESTADO_CIVIL ec ON ev.ID_ESTCIVIL_EST = ec.ID_ESTCIVIL
WHERE
    ev.FECTER_ESTCIVIL IS NULL OR eV.FECTER_ESTCIVIL > SYSDATE
